...
PROJECT_NAME=EASYR1 # 从 config.yaml 中获取的默认值
EXPERIMENT_NAME=qwen2_5_vl_7b_geo3k_grpo_dynamic_rubric_test
...
python3 -m verl.trainer.main \
    ...
    worker.reward.reward_function=/data2/chenyukun/workspace/EasyR1/examples/reward_function/dynamic_rubric.py:compute_score \
    worker.reward.reward_function_kwargs.max_concurrent=5 \
    worker.reward.reward_function_kwargs.timeout=60 \
    worker.reward.reward_function_kwargs.max_retries=2 \
    worker.reward.reward_function_kwargs.selector_max_concurrent=5 \
    worker.reward.reward_function_kwargs.selector_timeout=40 \
    worker.reward.reward_function_kwargs.selector_max_retries=2 \
    worker.reward.reward_function_kwargs.rubric_count=3 \
    trainer.n_gpus_per_node=4\
...
#     worker.actor.fsdp.torch_dtype=bf16 \
#     worker.ref.fsdp.torch_dtype=bf16 \
#     worker.reward.reward_function=/home/runjin.cyk/workspace/EasyR1/examples/reward_function/rubric_api.py:compute_score \
#     worker.reward.reward_function_kwargs.max_concurrent=20 \
#     worker.reward.reward_function_kwargs.timeout=60 \
#     worker.reward.reward_function_kwargs.max_retries=2 \
#     trainer.n_gpus_per_node=4\
...
#     trainer.logger='[file, tensorboard]'\
#     $@ 2>&1 | tee "verl_qwen2_5_vl_7b_geo3k_grpo_batchsize128_n8_api_test.log"
